package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1582:ColorTransform;
      
      private var var_2019:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_1582 = new ColorTransform();
         this.var_2019 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_110 = this.var_1319[param1];
         this.var_1042 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_2020 >= 0)
         {
            this.var_286 = this._visualization.getSprite(this.var_2020);
            if(this.var_286 && this.var_286.asset)
            {
               ...
               if(this.var_96 == null)
               {
                  this.var_96 = this.var_286.asset.clone();
                  if(this.var_849.alphaMultiplier != 1)
                  {
                     this.var_687 = new BitmapData(this.var_96.width,this.var_96.height,true,4278190080);
                  }
               }
               this.var_2782 = -this.var_286.offsetX;
               this.var_2781 = -this.var_286.offsetY;
               this.var_286.asset = this.var_96;
            }
            if(this.var_96)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_110.y * this.var_1320;
         }
         return 0;
      }
      ...
      {
         if(!this.var_110 || !this.var_286)
         {
            return;
         }
         if(this.var_96 && this.var_286.asset != this.var_96)
         {
            this.var_286.asset = this.var_96;
         }
         if(this.var_1042)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1042 && this.var_110.hasIgnited)
         {
            this.var_1042 = true;
         }
         _loc8_ = this.var_1264 * this.var_1320;
         this.var_110.update();
         if(this.var_1042)
         {
            if(this.var_110.roomObjectSpriteId >= 0)
            ...
            {
               this.var_96.draw(this.var_687,this.var_2019,this.var_849,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_110.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2782 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1320;
               _loc4_ = this.var_2781 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1320;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1582.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_96.draw(_loc7_,this._translationMatrix,this.var_1582,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2780 = parseInt(param1.@size);
         this.var_2020 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1264 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1320 = this.var_2780 / 64;
