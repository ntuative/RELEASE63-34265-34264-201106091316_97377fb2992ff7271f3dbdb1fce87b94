package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class MarketPlaceCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener, IMarketPlaceVisualization
   {
      ...
      private const const_2011:int = 2;
      
      private const const_2292:int = 40;
      
      private const const_2291:int = 10;
      
      private var var_814:Array;
      
      private var var_1539:MarketPlaceOfferData;
      ...
      override public function dispose() : void
      {
         ...
         if(this.var_551)
         {
            this.var_551.removeEventListener(TimerEvent.TIMER,this.onPopulationTimer);
            this.var_551 = null;
         }
      }
      ...
      private function updateList() : void
      {
         ...
         if(!this.var_660)
         {
            ...
            this.var_660 = _loc5_.buildFromXML(_loc4_) as IWindowContainer;
         }
         if(!this.var_660)
         {
            return;
         ...
         this.updateStatusDisplay(this.const_2011,_loc3_.length,_loc2_);
         if(!this.var_551)
         {
            this.var_551 = new Timer(25);
            this.var_551.addEventListener(TimerEvent.TIMER,this.onPopulationTimer);
         }
         this.var_1281 = 0;
         this.populateList();
         this.var_551.start();
      ...
      private function populateList() : Boolean
      {
         ...
         while(_loc1_ < 5)
         {
            if(this.var_1281 >= this._offers.length)
            {
               return true;
            }
            this.addListItem(this._offers.getWithIndex(this.var_1281));
            ++this.var_1281;
            _loc1_++;
         }
         return false;
      ...
      {
         var _loc5_:* = null;
         this.updateStatusDisplay(this.const_2012);
         var _loc1_:int = -1;
         var _loc2_:int = -1;
         var _loc3_:String = "";
         var _loc4_:int = 1;
         ...
         else if(param1.type == WindowEvent.const_110)
         {
            ...
            switch(_loc7_.name)
            {
               ...
               case "max_price_input":
                  _loc8_ = this.const_2291;
                  break;
               case "search_input":
                  _loc8_ = this.const_2292;
                  break;
               default:
                  return;
            }
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            switch(param2.name)
            {
               case "back_button":
                  ...
                  break;
               case "buy_button":
                  this.marketPlace.buyOffer(this.var_1539.offerId);
            }
            return;
         }
         if(param1.type == WindowEvent.const_49)
