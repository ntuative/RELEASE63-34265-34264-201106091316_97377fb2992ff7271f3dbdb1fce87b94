package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_334:String = "failure";
      ...
      private var var_1652:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1653 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1652.init();
            this.var_1652.addEventListener(HabboWeb.const_285,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1653;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1653);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1653);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1652.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_2121 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_2053 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2941 = Boolean(facebookData[1]);
               }
               if(this.var_2121.length > 0)
               {
