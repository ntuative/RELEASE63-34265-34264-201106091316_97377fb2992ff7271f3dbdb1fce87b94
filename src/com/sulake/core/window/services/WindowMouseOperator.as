package com.sulake.core.window.services
{
   ...
   public class WindowMouseOperator implements IDisposable
   {
      ...
      protected var var_164:DisplayObject;
      ...
      public function begin(param1:IWindow, param2:uint = 0) : IWindow
      {
         ...
         {
            this.var_164.addEventListener(MouseEvent.MOUSE_DOWN,this.handler,false);
            this.var_164.addEventListener(MouseEvent.MOUSE_UP,this.handler,false);
            this.var_164.addEventListener(Event.ENTER_FRAME,this.handler);
            this._mouse.x = this.var_164.mouseX;
            this._mouse.y = this.var_164.mouseY;
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            ...
            {
               this.var_164.removeEventListener(MouseEvent.MOUSE_DOWN,this.handler,false);
               this.var_164.removeEventListener(MouseEvent.MOUSE_UP,this.handler,false);
               this.var_164.removeEventListener(Event.ENTER_FRAME,this.handler);
      ...
      protected function handler(param1:Event) : void
      {
         ...
         if(this._working)
         {
            if(param1.type == Event.ENTER_FRAME)
            {
               ...
               else if(this._mouse.x != this.var_164.mouseX || this._mouse.y != this.var_164.mouseY)
               {
                  this.operate(this.var_164.mouseX,this.var_164.mouseY);
                  this._mouse.x = this.var_164.mouseX;
                  this._mouse.y = this.var_164.mouseY;
               }
               return;
      ...
      public function operate(param1:int, param2:int) : void
      {
         ...
         this.getMousePositionRelativeTo(this._window,this._mouse,this.var_735);
         this._window.offset(this.var_735.x - this._offset.x,this.var_735.y - this._offset.y);
      }
